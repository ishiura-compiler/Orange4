...
sub top_down_prepare {
    ...
    if ( !$self->_search_assigns_var_and_exist_check( $name, $assign_var_locate ) ) {
...
sub _search_assigns_var_and_exist_check {
    ...
    return Orange4::Mini::Util::search_assigns_var( $self->{assigns}, $name,
        $assign_var_locate );
}
