...
If the Orange4 does not work correctly, please review the
following settings.
END_OF_DATA
        print $caution;
    }
...
/^((signed|unsigned) (char|short|int|long|long long)|(float|double|long double))$/
        )
        {
            ...
            if ( !defined $type_->{order} || $type_->{order} !~ /^[0-9]+$/ ) {
            ...
            elsif ( !defined $type_->{bits} || $type_->{bits} !~ /^[0-9]+$/ ) {
                $caution .= _c_msg("Bit of type '$type' is wrong.");
            }
            elsif ( $type =~ /^(unsigned|signed)/ && ( $type_->{bits} % 4 ) != 0 ) {
                $caution .= _c_msg("Bit of type '$type' is not divisible by 4.");
            }
            elsif ( !defined $type_->{min} || $type_->{min} !~ /^[+-]?[0-9]+$/ ) {
                $caution .= _c_msg("Min of type '$type' is wrong.");
            }
            elsif ( !defined $type_->{max} || $type_->{max} !~ /^[+-]?[0-9]+$/ ) {
...
sub _check_config_modifiers {
    ...
    foreach my $modifier ( @{ $self->{modifiers} } ) {
        if ( $modifier eq "" || $modifier =~ /(const|volatile)/ ) {
            ;
        }
        else {
...
sub _check_config_macro_ng {
    my $self = shift;
    if ( !defined $self->{macro_ng} || $self->{macro_ng} eq "" ) {
        return _c_msg("Undefined source_file.");
    }
...
sub _check_config_macro_ok {
    my $self = shift;
    if ( !defined $self->{macro_ok} || $self->{macro_ok} eq "" ) {
        return _c_msg("Undefined source_file.");
    }
...
sub _check_config_exec_file {
    my $self = shift;
    if ( !defined $self->{exec_file} || $self->{exec_file} eq "" ) {
        return _c_msg("Undefined source_file.");
    }
...
sub _check_config_source_file {
    my $self = shift;
    if ( !defined $self->{source_file} || $self->{source_file} eq "" ) {
        return _c_msg("Undefined source_file.");
    }
...
sub _check_config_e_size_num {
    ...
    elsif ( $self->{e_size_num} < 1 || $self->{e_size_num} !~ /^[0-9]+$/ ) {
        return _c_msg("! e_size_num is wrong.");
    }
    elsif ( $self->{e_size_num} > 0 && $self->{e_size_num} < 10000 ) {
        return "";
    }
    else {
