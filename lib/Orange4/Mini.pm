...
sub _check_argument {
  ...
  for my $path ( @{ $self->{argv} } ) {
    if ( Orange4::Util::is_file($path) ) {
      $self->{target_file} = $path;
    }
    elsif ( Orange4::Util::is_dir($path) ) {
...
sub _load_configs {
  ...
  my $config_file =
    File::Spec->catfile( $self->{target_directory}, 'Orange4.cnf' );
  $self->{config} = Orange4::Config->new($config_file);

  my $compiler_cnf =
    File::Spec->catfile( $self->{target_directory}, 'Orange4-compiler.cnf' );
  my $executor_cnf =
    File::Spec->catfile( $self->{target_directory}, 'Orange4-executor.cnf' );
